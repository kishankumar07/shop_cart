...
const isLogged = (req, res, next) => {
    ...
    if (req.session.user) {
        ...
        // const user = User.findById(req.session.user).lean(): Attempts to find a user in the database using the findById method, based on the user ID stored in the session. The lean() method is used to obtain a plain JavaScript object rather than a Mongoose document.
        if (user) {
